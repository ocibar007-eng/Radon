# PROMPT DEFINITIVO (ESPECIFICAÇÃO) — LAUDO DE ULTRASSONOGRAFIA DE ABDOME TOTAL (V1.4)

---

## LEIA PRIMEIRO (USO NO DIA A DIA)

Este arquivo é a **especificação completa** (documento “mãe”) para manutenção e padronização.

Para gerar laudos no dia a dia (rápido e com menor risco de quebra de formatação), use:

- `USG_ABD_TOTAL_RUNTIME.md` (execução)
- `CALCULOS_MEDICOS.md` (cálculos, quando necessário)
- `PEDIATRIA_REFERENCIAS/USG_ABDOME_REFERENCIAS_RAPIDAS.md` (quando pediátrico)

Este arquivo (`USG_ABD_TOTAL_COMPLETO.md`) deve ser usado para:

- Ajustar regras quando algo “não dá certo” (formatação, órgão faltando, pelve, Doppler, medidas).
- Manter documentação das exceções, gates e checklist.
- Sincronizar mudanças pontuais para o runtime (somente o essencial).

---

## CONTRATO DE SAÍDA (INVIOLÁVEL)

1) O texto de saída deve começar **diretamente** com o separador `---` do laudo (sem cabeçalhos, sem “✓ arquivos lidos…”, sem comentários).

2) Depois do laudo completo, gerar um bloco separado:

AUDITORIA INTERNA (NÃO COPIAR PARA O LAUDO)

3) É proibido exibir “passo a passo”, “lendo documentos”, raciocínio interno ou qualquer meta-texto de processo.

4) É proibido pesquisar na internet para valores normativos (“literatura” = somente referências locais do projeto).

---

## DIFERENÇAS VS `USG_ABD_TOTAL_RUNTIME.md`

- O runtime é propositalmente **curto** para reduzir tempo e risco de erro; ele contém apenas o necessário para executar.
- Este arquivo contém: regras detalhadas, checklist, parsing/gates, notas e um apêndice de referências locais.
- Quando houver divergência, a execução deve seguir o runtime; as correções devem ser feitas aqui e depois propagadas para o runtime de forma mínima.
...
Nota de manutenção: a fonte canônica é o arquivo em `PEDIATRIA_REFERENCIAS/USG_ABDOME_REFERENCIAS_RAPIDAS.md`. Se houver atualização lá, copie/atualize este bloco apenas se realmente precisar manter a versão “tudo em um”.
...
| 0,25 - 0,5 | 7,3 (1,1) | 4,5 - 9,5 |
| 0,5 - 0,75 | 7,9 (0,8) | 6,0 - 10,0 |
