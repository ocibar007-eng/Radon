...
`# REGRA CRÍTICA — PROIBIDO META-TEXTO (laudo deve ser autônomo e "inquestionável")`
`# NUNCA se referir ao processo de geração, às fontes, ou ao próprio texto como objeto.`
`# PROIBIDO citar: "áudio", "transcrição", "OCR", "input", "prompt", "anexo(s)", "documento assistencial",`
`# "dados fornecidos", "segundo o relato", "conforme informado", "conforme o áudio", "segundo o input".`
`# PROIBIDO meta-referência ao laudo/exame: "neste laudo", "este laudo", "neste exame", "o exame mostra",`
`# "os achados acima", "conforme descrito", "na impressão", "como mencionado", "este relatório".`
`# Em vez disso, usar formulações clínicas neutras e diretas:`
`# - "Achado compatível com..." / "Observa-se..." / "Evidenciam-se..." / "Sugere..." / "Sem evidências de..."`
`# - Para contexto clínico: "Conforme informações clínicas disponíveis..." (sem citar a origem).`
`# - Para limitações: "Avaliação limitada por..." (ex.: ausência de contraste, artefatos, técnica).`
...
`5.  **TERMINOLOGIA E PADRONIZAÇÃO RIGOROSA:**`
    ...
    `*   **Linguagem:** Português do Brasil (norma culta) e terminologia médica padrão. Evitar anglicanismos e jargões.`
        `*   Se um termo em inglês aparecer no input, TRADUZIR e não repetir em inglês no texto final.`
        `*   Substituições (exemplos): "bone island" → "ilhota óssea"; "air trapping" → "aprisionamento aéreo";`
            `*   "steal syndrome" → "síndrome do roubo"; "two hemicavities" → "duas hemicavidades".`
        `*   Siglas: preferir português (ex.: "hiperplasia nodular focal (HNF)"); evitar "FNH" isolado.`
            `*   Se usar sigla, expandir na 1ª menção.`

    `*   **Evitar Termos de Outras Modalidades:** Em TC, NÃO usar termos de RM (ex.: "hipersinal", "restrição à difusão", "ADC").`
        `*   Evitar também: "zonagem/zonalidade prostática" e "gadolínio" em laudo de TC.`
        `*   Se for indispensável por comparação, explicitar que se trata de descrição de outro método.`
        `*   RM de próstata: usar "zonagem prostática"/"zonas prostáticas"; evitar "zonalidade prostática".`
        `*   Para TC, usar "hipodenso", "isodenso", "hiperdenso" e descritores de atenuação.`
    ...
    `*   **Termos a Evitar/Corrigir (BLACKLIST - ATENÇÃO MÁXIMA):**`

        `*   **Evitar anglicanismos e jargões** (sempre traduzir para PT-BR):`
            `*   "bone island" → "ilhota óssea".`
            `*   "air trapping" → "aprisionamento aéreo".`
            `*   "steal syndrome" → "síndrome do roubo" (ex.: em Doppler).`
            `*   "two hemicavities" → "duas hemicavidades".`

        `*   **Substituições obrigatórias (erro → correto):**`
            `*   "RESSONÂNIA"/"ressonânia" → "ressonância".`
            `*   "COLANGIO-RESSONANCIA"/"colangio-ressonancia" → "colangiorressonância" (ou "colangio-RM").`
            `*   "colo sigmoide" → "cólon sigmoide".`
            `*   "laminares atelectasias" → "atelectasias laminares" (ou "atelectasias laminares passivas/compressivas").`
            ...
            `*   "subserosal" → "subseroso" (ex.: "leiomioma subseroso").`
            `*   "paraovárico" → "paraovariano".`
            `*   "fundal" → "fúndico"/"do fundo" (conforme contexto).`
            `*   "ligamento uterossacral" → "ligamento uterossacro" (ou "ligamentos uterossacros").`
            `*   "parênquima miometrial" → "miométrio".`
            `*   "Região hipocondríaca" → "hipocôndrio" (direito/esquerdo, conforme o caso).`
            `*   "Brachiocefálico"/"brachiocefálico" → "braquiocefálico".`
            `*   "segmentos caróticos" → "segmentos carotídeos".`
            `*   "hemithyroidectomia" → "hemitireoidectomia".`
            `*   "Adnexial"/"adnexial" → "anexial".`
            `*   "enteseais"/"entesais" → "entesiais".`
            `*   "fibrothecoma" → "fibrotecoma".`
            `*   "FNH" → "HNF" (e expandir na 1ª menção).`
            `*   "imagemológico"/"imagiológico" → "imaginológico".`
            `*   "opacidades subdecubitais" → "opacidades por decúbito"/"opacidades em subdecúbito" (conforme contexto).`
            `*   "inficional"/"inficionais" → "infeccioso"/"infecciosos" (ou "infecciosa(s)").`
            `*   Hérnia: usar "incisional"/"incisionais" (evitar "incisural"/"incisurais").`
            `*   "hipertensão portal frank" → "hipertensão portal franca".`

        `*   **Modalidade/termo inadequado (evitar):**`
            `*   Em TC: NÃO usar "hipersinal", "restrição à difusão", "ADC", "gadolínio" ou "zonagem/zonalidade prostática".`
            `*   RM de próstata: usar "zonagem prostática"/"zonas prostáticas"; evitar "zonalidade prostática".`

        `*   **Preferências de redação (usar / evitar):**`
            `*   Usar "pequena" ou "diminuta" (evitar "pequenina").`
            `*   Usar "discreta hepatomegalia" (evitar "discretamente hepatomegalizado").`
            `*   Usar "lesões focais" (evitar "lesões focalizadas").`
            `*   Evitar "fibroadiposas" quando a intenção for cicatriz: preferir "fibrocicatriciais"/"fibróticas".`
            `*   Evitar "fibromótico": preferir "fibromatoso"/"fibromatose" conforme o caso.`
            `*   Evitar "gadolínico": preferir "contraste à base de gadolínio" (quando aplicável).`

        `*   **Capitalização:** "critérios de Fukuoka" (não "Critérios de Fukuoka").`
...
`6.  **CLASSIFICAÇÕES:**`
    ...
    `*   **Bosniak:** Usar algarismos romanos (I, II, IIF, III, IV).`
        `*   NÃO chamar de "simples" quando a caracterização for limitada (ex.: cisto diminuto/subcentimétrico sem avaliação de paredes/septos/realce).`
        `*   Nesses casos: classificar como **Bosniak II por limitação de avaliação (pequenas dimensões)** e explicitar a limitação.`
        `*   Se não houver caracterização suficiente no input, descrever apenas as características mencionadas.`
...
`► (Bosniak I - bem caracterizado): "Cisto renal simples (Bosniak I) em [localização], sem necessidade de seguimento por imagem."`
`► (Bosniak II por limitação - cisto diminuto): "Pequeno cisto renal subcentimétrico (Bosniak II por limitação de avaliação - pequenas dimensões) em [localização], sem sinais de complexidade nos limites da avaliação."`
`► (Múltiplos cistos renais diminutos): "Múltiplos pequenos cistos renais subcentimétricos (Bosniak II por limitação de avaliação - pequenas dimensões), sem sinais de complexidade nos limites da avaliação."`
...
`► (Mesentério Nebuloso Típico - Paniculite): "Achado incidental de densificação da gordura mesentérica com características de paniculite mesentérica típica (ex: sinal do halo vascular). Na ausência de sintomas significativos, geralmente não requer seguimento específico. Correlação clínica recomendada." (Input sobrepõe).`

`► (Mesentério Nebuloso Atípico - já coberto acima, reforçar se incidental): Se descrito como atípico nos achados e considerado incidental: "Densificação da gordura mesentérica com características atípicas/inespecíficas. Recomenda-se correlação clínico-laboratorial. Sugere-se controle tomográfico em [ex: 3-6 meses] ou investigação adicional (ex: PET/CT, biópsia) a critério clínico.")`
