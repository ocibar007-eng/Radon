...
import { isClaudeAvailable } from '../src/adapters/anthropic/index.ts';
import type { StreamReportRequest } from '../src/server/claude-draft-stream.ts';
import { generateDraftReportStream } from '../src/server/claude-draft-stream.ts';
...
export default async function handler(req: any, res: any) {
  ...
  if (!CONFIG.CLAUDE_REPORT_ENABLED) {
    res.status(403).json({ error: 'Claude report generation is disabled.' });
    return;
  }
