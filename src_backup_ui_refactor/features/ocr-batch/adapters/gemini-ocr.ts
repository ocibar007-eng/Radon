/**
 * Gemini OCR Adapter - Usa o cliente centralizado do Radon
 * IMPORTANTE: Não cria nova instância de GoogleGenAI, reutiliza getGeminiClient()
 */
...
export const runGeminiOcr = async (file: File): Promise<BatchOcrResult> => {
    ...
    if (!ai) {
        throw new Error(
            '❌ Cliente Gemini não inicializado. Verifique se API_KEY está configurada no .env'
        );
    }
  ...
  `;
    ...
    while (true) {
        try {
            ...
            // API v1.35+ - usa response.text diretamente
